  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const UserUpdate = data_id => {
        window.location = '/Owner/Editdb_data/' + data_id
    }

    const Process_owner = data_id => {
        window.location = '/Owner/Process_owner/' + data_id
    }
    const Process_divide = data_id => {
        window.location = '/Owner/Process_divide/' + data_id
    }
    const Process_percent = data_id => {
        window.location = '/Owner/Process_percent/' + data_id
    }
    ...
    const UserDelete = data_id => {
        ...
        fetch("https://latexplatform-api.coecore.com/db_data_id", requestOptions)
            ...
            .then((data) => {
                console.log(data)
                if (data.status === 'Ok') {
                    window.location = '/Owner/datadisplay'
