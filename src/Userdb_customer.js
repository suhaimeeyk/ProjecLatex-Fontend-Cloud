...
export default function Users() {
  ...
  const UserGet = () => {
    fetch("https://latexplatform-api.coecore.com/db_customer")
      .then(res => res.json())
      .then(
        (result) => {
          setItems(result);
  ...
  const filteredData = items.results?.filter((results, index) => {
    ...
    const customer_tel = results.customer_tel.toLowerCase();
    const catusers_name = results.catusers_name.toLowerCase();
